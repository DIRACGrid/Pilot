...
if os.environ.get('READTHEDOCS') == 'True':
  ...
  pilotPath = os.path.abspath( os.path.join( os.getcwd(), "../..") )
  print "pilotPath", pilotPath
  ...
  if not os.path.exists( "../../Pilot" ):
    pilotLink =  os.path.abspath( os.path.join( os.getcwd()  , "../" , buildfolder, "Pilot" ) )
    print "pilotLink",pilotLink
    if not os.path.exists( pilotLink ):
      RES = subprocess.check_output( ["ln","-s", pilotPath, pilotLink ] )
    pilotPath = os.path.abspath( os.path.join( pilotLink, ".." ) )

  sys.path.insert(0, pilotPath)

  for path in sys.path:
    os.environ['PYTHONPATH'] = os.environ.get('PYTHONPATH', '')+":"+path

  os.environ["Pilot"] = pilotPath
  print "Pilot ENVIRON", os.environ["Pilot"]
...
project = u'Pilot'
copyright = u'%s, Pilot Project' % datetime.datetime.utcnow().year
...
html_title = "Pilot Documentation"
...
htmlhelp_basename = 'PilotDocsdoc'
...
latex_documents = [
  ('index', 'PilotDocs.tex', u'Pilot Documentation',
   u'Pilot Project.', 'manual'),
]
