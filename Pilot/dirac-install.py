...
class ReleaseConfig(object):
    ...
    def loadProjectRelease(self, releases, project=None, sourceURL=None, releaseMode=None, relLocation=None):
        """
    ...
    def getModulesForRelease(self, release, project=None):
        ...
        if modules:
            ...
            if modules:
                modules = dict(
                    (modName.strip(), release) for modName in modules.split(",") if modName.strip()
                )  # pylint: disable=no-member
            else:
    ...
    def getModSource(self, release, modName, project=None):
        ...
        if self.projectName not in self.prjRelCFG:
            return S_ERROR("Project %s has not been loaded. I'm a MEGA BUG! Please report me!" % self.projectName)
    ...
    def getModulesToInstall(self, release, extensions=None):
        ...
        if release not in self.prjDepends[self.projectName]:
            return S_ERROR("Version %s has not been loaded for project %s" % (release, self.projectName))
...
def downloadAndExtractTarball(tarsURL, pkgName, pkgVer, checkHash=True, cache=False):
    ...
    if os.path.isfile(postInstallScript):
        ...
        if os.system("python '%s' > '%s.out' 2> '%s.err'" % (postInstallScript, postInstallScript, postInstallScript)):
            logERROR("Post installation script %s failed. Check %s.err" % (postInstallScript, postInstallScript))
    return True
...
def loadConfiguration():
    ...
    """
    optList, args = getopt.getopt(sys.argv[1:], "".join([opt[0] for opt in cmdOpts]), [opt[1] for opt in cmdOpts])
    ...
    for opName in ("release", "debug", "globalDefaults", "targetPath", "project", "release", "extensions", "timeout"):
        try:
...
def createBashrcForDiracOS():
    ...
    try:
        ...
        if not os.path.exists(bashrcFile):
            ...
            lines.extend(["# Some DIRAC locations", "export DIRACSCRIPTS=%s" % os.path.join("$DIRAC", "scripts")])
            ...
            lines.extend(["# IPv6 support", "export GLOBUS_IO_IPV6=TRUE", "export GLOBUS_FTP_CLIENT_IPV6=TRUE"])
    ...
    for modName in set(modsOrder):
        ...
        if not os.path.isfile(ddeLocation):
            ddeLocation = os.path.join(
                cliParams.targetPath, "DIRAC", "src", "DIRAC", "Core", "scripts", "dirac_deploy_scripts.py"
            )
