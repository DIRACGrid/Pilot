...
def createPilotLoggerConfigFile(
    filename="PilotLogger.json",
    loggingType="",
    localOutputFile="",
    host="",
    port="",
    ...
    values = [loggingType, localOutputFile, host, port, url, key_file, cert_file, ca_certs, fileWithID, queue]
...
def generateDict(pilotUUID, timestamp, source, phase, status, messageContent):
    ...
    keys = ["pilotUUID", "timestamp", "source", "phase", "status", "messageContent"]
...
def isMessageFormatCorrect(content):
    ...
    if not isinstance(content, dict):
        return False
    refKeys = sorted(["pilotUUID", "status", "messageContent", "timestamp", "phase", "source"])
