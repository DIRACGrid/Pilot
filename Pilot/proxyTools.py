...
def getVO(proxy_data):
    ...
    for cert in chain:
        ...
        if idx_voms_line is None:
            continue
        voms_extension = parseASN1(
            b16decode(cert_info[idx_voms_line + 1].split(b":")[-1])
        )
        # Look for the attribute names
        idx_fqans = findExtension(VOMS_FQANS_OID, voms_extension)
        (initial_depth,) = map(
            int, RE_OPENSSL_ANS1_FORMAT.match(voms_extension[idx_fqans - 1]).groups()
        )
...
def extract_diracx_payload(proxy_data):
    ...
    match = re.search(
        r"-----BEGIN DIRACX-----(.*?)-----END DIRACX-----", proxy_data, re.DOTALL
    )
    if not match:
...
class BaseRequest(object):
    ...
    def __init__(self, url, caPath, pilotUUID, name="unknown"):
        ...
        self.url = url
        self.caPath = caPath
        self.headers = {"User-Agent": "Dirac Pilot [Unknown ID]"}
    ...
    def executeRequest(
        self, raw_data, insecure=False, content_type="json", json_output=True
    ):
    ...
    def __execute_raw_request(
        ...
        if content_type == "json":
            ...
            if content_type == "x-www-form-urlencoded":
                if sys.version_info.major == 3:
                    data = urlencode(raw_data).encode(
                        "utf-8"
                ...
                raise ValueError(
                    "Invalid content_type. Use 'json' or 'x-www-form-urlencoded'."
                )
        ...
        if json_output:
            try:
                ...
                ValueError
            ):  # In Python 2, json.JSONDecodeError is a subclass of ValueError
...
class TokenBasedRequest(BaseRequest):
    ...
    def __init__(self, diracx_URL, endpoint_path, caPath, jwtData, pilotUUID):
        ...
        super(TokenBasedRequest, self).__init__(
            url, caPath, pilotUUID, "TokenBasedConnection"
        )
...
class X509BasedRequest(BaseRequest):
    ...
    def __init__(self, url, caPath, certEnv, pilotUUID):
        super(X509BasedRequest, self).__init__(
            url, caPath, pilotUUID, "X509BasedConnection"
        )
        ...
        try:
            ...
            self._context.load_cert_chain(
                os.path.join(self.certEnv, "hostcert.pem"),
                os.path.join(self.certEnv, "hostkey.pem"),
            )
...
def refreshPilotToken(url, pilotUUID, jwt, _=None):
    ...
    # Perform the request to refresh the token
    response = config.executeRequest(
        raw_data={"refresh_token": jwt["refresh_token"], "pilot_stamp": pilotUUID},
        insecure=True,
    )
...
def revokePilotToken(url, pilotUUID, jwt, clientID):
    ...
    # Create request object with required configuration
    config = BaseRequest(
        url="%sapi/auth/revoke" % url, caPath=caPath, pilotUUID=pilotUUID
    )

    # Prepare refresh token payload
    payload = {"refresh_token": jwt["refresh_token"], "client_id": clientID}

    # Perform the request to revoke the token
    _response = config.executeRequest(
        raw_data=payload, insecure=True, content_type="query", json_output=False
    )
