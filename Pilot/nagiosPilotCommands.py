...
class NagiosProbes( CommandBase ):
  ...
  def __init__( self, pilotParams ):
    ...
    self.nagiosProbes = [ 'WN-sft-csh', 'WN-sft-vo-swdir', 'WN-sft-voms', 'WN-mjf' ]
    self.nagiosPutURL = 'https://lbvobox06.cern.ch:9132/sam/'

  def _setNagiosOptions( self ):
    """ Setup installation parameters
    """
    
    # FIXME: not listed in pilotTools so not usable...
    
    for o, v in self.pp.optList:
      if o == '--nagiosProbes':
        self.nagiosProbes.append( v.split( ',' ) )
      elif o == '--nagiosPutURL':
        self.nagiosPutURL = v
